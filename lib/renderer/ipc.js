...
Ipc.sendToMain = function (message, ...args) {
  ...
  if ( opts ) {
    ...
    let sessionId = _addSession(`${winID}@renderer`, opts.reply, opts.timeout);
...
Ipc.sendToPanel = function (panelID, message, ...args) {
  ...
  ipcRenderer.send.apply( ipcRenderer, ['editor:ipc-renderer2panel', panelID, message, ...args] );
};
...
function _registerReplyIpc ( message ) {
  ...
  // already registered
  if ( ipcRenderer.listeners(replyMessage).length > 0 ) {
    return;
  }

  ipcRenderer.on(replyMessage, (event, ...args) => {
...
function _addSession ( prefix, fn, timeout ) {
