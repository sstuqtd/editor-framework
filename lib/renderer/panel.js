...
const EditorR = require('./editor'); // DELME
const Console = require('./console');
const Ipc = require('./ipc');
const i18n = require('./i18n');
const IpcListener = require('./ipc-listener');
...
Panel.load = function ( panelID, cb ) {
  Ipc.sendRequestToCore('panel:query-info', panelID, ( panelInfo ) => {
    ...
    EditorR.UI.import( framePath, ( err ) => {
      if ( err ) {
      ...
      let frameCtor = Panel.panels[panelID];
      if ( !frameCtor ) {
...
Panel.closeAll = function ( cb ) {
  ...
  let rootEL = EditorR.UI.DockUtils.root;
  if ( rootEL ) {
    rootEL.remove();
    EditorR.UI.DockUtils.root = null;
  }
...
Panel.dumpLayout = function () {
  let root = EditorR.UI.DockUtils.root;
  if ( !root ) {
    return null;
  }
...
//     var root = EditorR.UI.DockUtils.root;
//     if ( !root ) {
//         return null;
//     }
//     if ( !root._dockable ) {
