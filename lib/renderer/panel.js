...
Panel.load = function ( panelID, cb ) {
  Ipc.sendToMain('panel:query-info', panelID, ( info ) => {
    if ( !info ) {
    ...
    UI.PolymerUtils.import( framePath, ( err ) => {
      ...
      Ipc.sendToMain('panel:dock', panelID);
      ...
      if ( info.messages.indexOf('panel:run') === -1 ) {
        info.messages.push('panel:run');
      }
...
Panel.open = function ( panelID, argv ) {
  Ipc.sendToMain('panel:open', panelID, argv);
};
...
Panel.popup = function ( panelID ) {
  ...
  if ( panelCounts > 1 ) {
    Panel.close(panelID);
    Ipc.sendToMain('panel:open', panelID);
  }
};

Panel.close = function ( panelID ) {
  Panel.undock(panelID);
  Ipc.sendToMain('panel:close', panelID);
};
...
Panel.closeAll = function ( cb ) {
  ...
  for ( let i = 0; i < panelIDs.length; ++i ) {
    Ipc.sendToMain('panel:wait-for-close', panelIDs[i], () => {
      --finishCount;
      if ( finishCount === 0 && cb ) {
        cb();
      }
