(() => {
  ...
  let Droppable = {
    ...
    _initDroppable ( dropAreaElement ) {
      ...
      dropAreaElement.addEventListener('dragenter', event => {
        ...
        if ( this._dragenterCnt === 1 ) {
          this.checkIfDroppable(event.dataTransfer, ( droppable, dragType, dragItems ) => {
            ...
            EditorUI.fire(event.target, 'drop-area-enter', {
              bubbles: true,
              detail: {
                dragType: dragType,
                dragItems: dragItems,
      ...
      dropAreaElement.addEventListener('dragleave', event => {
        ...
        if ( this._dragenterCnt === 0 ) {
          this.checkIfDroppable( event.dataTransfer, ( droppable, dragType, dragItems ) => {
            ...
            EditorUI.fire(event.target, 'drop-area-leave', {
              bubbles: true,
              detail: {
                dragType: dragType,
                dragItems: dragItems,
      ...
      dropAreaElement.addEventListener('drop', event => {
        ...
        this.checkIfDroppable( event.dataTransfer, ( droppable, dragType, dragItems ) => {
          ...
          EditorUI.fire(event.target, 'drop-area-accept', {
            bubbles: true,
            detail: {
              dragType: dragType,
              dragItems: dragItems,
    ...
    checkIfDroppable ( dataTransfer, fn ) {
      let droppableList = this.droppable.split(',');
      let dragType = EditorUI.DragDrop.type(dataTransfer);
      ...
      let dragItems = EditorUI.DragDrop.items(dataTransfer);
