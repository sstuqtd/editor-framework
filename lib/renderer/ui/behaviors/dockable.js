...
let Dockable = {
  ...
  removeDock ( childEL ) {
    ...
    if ( this.children[0] === childEL ) {
      if ( childEL.nextElementSibling && DockUtils.isResizer(childEL.nextElementSibling) ) {
        this.removeChild(childEL.nextElementSibling);
      }
    } else {
      if ( childEL.previousElementSibling && DockUtils.isResizer(childEL.previousElementSibling) ) {
