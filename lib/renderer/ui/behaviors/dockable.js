  ...
  let Dockable = {
    ...
    addDock ( position, element ) {
      ...
      if ( parentEL._dockable ) {
        ...
        if ( position === 'left' || position === 'right' ) {
          ...
          newWidth = Math.max( 0, rect.width-element.curWidth-EditorUI.DockUtils.resizerSpace );
        } else {
          if ( parentEL.row ) {
            needNewDock = true;
          }
          newHeight = Math.max( 0, rect.height-element.curHeight-EditorUI.DockUtils.resizerSpace );
        }

        // process dock
        if ( needNewDock ) {
          // new <editor-dock>
          newDock = document.createElement('editor-dock');
          ...
          newResizer = document.createElement('editor-dock-resizer');
    ...
    removeDock ( childEL ) {
      ...
      if ( this.children[0] === childEL ) {
        if ( childEL.nextElementSibling.tagName === DockResizer.tagName ) {
          this.removeChild(childEL.nextElementSibling);
        }
      } else {
        if ( childEL.previousElementSibling.tagName === DockResizer.tagName ) {
