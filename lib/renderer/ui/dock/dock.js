(() => {
  ...
  class Dock extends window.HTMLElement {
    ...
    createdCallback () {
      ...
      root.innerHTML = `
        <div class="content">
          <content select="editor-dock,editor-panel,editor-dock-resizer"></content>
        </div>
      `;
      root.insertBefore(
    ...
    reflow () {
      ...
      for ( let i = 0; i < this.children.length; ++i ) {
        ...
        // DISABLE: let size = Math.round(this.row ? rect.width : rect.height);
        let size = this.row ? rect.width : rect.height;
        sizeList.push(size);
