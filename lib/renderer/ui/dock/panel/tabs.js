...
class Tabs extends window.HTMLElement {
  static get tagName () { return 'EDITOR-TABS'; }
  ...
  createdCallback () {
    ...
    `;
    root.insertBefore(
      ResUtils.createStyleElement('editor-framework://lib/renderer/ui/css/tabs.css'),
      root.firstChild
    );
  ...
  _onMenuPopup ( event ) {
    ...
    Editor.Menu.popup([
      { label: Editor.T('PANEL_MENU.maximize'), dev: true, message: 'panel:maximize', params: [panelID] },
      { label: Editor.T('PANEL_MENU.pop_out'), message: 'panel:popup', enabled: popable, params: [panelID] },
...
_.assign(Tabs.prototype, Droppable);
