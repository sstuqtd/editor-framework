(() => {
  ...
  function _isDockResizer (el) {
    return el && el.tagName === 'EDITOR-DOCK-RESIZER';
  }
  ...
  class DockResizer extends window.HTMLElement {
    createdCallback () {
      ...
      `;
      root.insertBefore(
        EditorUI.createStyleElement('editor-framework://lib/renderer/ui/css/resizer.css'),
        root.firstChild
      );
    ...
    _snapshot () {
      ...
      for ( let i = 0; i < parentEL.children.length; ++i ) {
        ...
        // DISABLE: sizeList.push( Math.round(this.vertical ? rect.width : rect.height) );
  ...
  EditorUI.isDockResizer = _isDockResizer;
  document.registerElement('editor-dock-resizer', DockResizer);
