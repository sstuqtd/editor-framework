...
class ColorPickerElement extends window.HTMLElement {
  ...
  _initEvents () {
    ...
    this._sliderR.addEventListener('change', event => {
      this._value[0] = event.detail.value * 255;
      this._updateHue();
      this._updateColor();
      this._updateAlpha();
      this._updateHexInput();
    ...
    this._sliderG.addEventListener('change', event => {
      this._value[1] = event.detail.value * 255;
      this._updateHue();
      this._updateColor();
      this._updateAlpha();
      this._updateHexInput();
    ...
    this._sliderB.addEventListener('change', event => {
      this._value[2] = event.detail.value * 255;
      this._updateHue();
      this._updateColor();
      this._updateAlpha();
      this._updateHexInput();
  ...
  _updateColor () {
    ...
    this._colorCtrl.style.backgroundColor = Chroma(hsv[0],1,1,'hsv').hex();
    this._colorHandle.style.left = `${hsv[1]*100}%`;
    this._colorHandle.style.top = `${(1-hsv[2])*100}%`;
  }
  ...
  _updateHexInput () {
    this._hexInput.value = Chroma(this._value).hex();
  }
}
