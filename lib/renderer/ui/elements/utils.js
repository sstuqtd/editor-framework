...
Utils.regenProperty = function ( propEL, opts, cb ) {
...
function _doRegen ( propEL, proto, opts, cb ) {
  let content;

  // NOTE: if regen without clear, it means we wish to use the children element and pass them to ready()
  if ( opts.clear ) {
    DomUtils.clear(propEL);
  } else {
    let userContent = propEL.querySelector('.user');
