...
let PropElement = ElementUtils.registerElement('ui-prop', {
  ...
  set tooltip (val) {
    if (this._tooltip !== val) {
      ...
      if ( this._tooltip ) {
        if ( !this.$tooltip ) {
          let el = new Hint(this._tooltip);
          this.$label.appendChild(el);
          this.$tooltip = el;
          this.$tooltip.style.display = 'none';
          this.$tooltip.style.position = 'absolute';
          this.$tooltip.style.top = '-35px';
          this.$tooltip.classList = ['bottom'];
          this.$tooltip.position = '20px';
        }
        this.$tooltip.innerText = this._tooltip;
      }
    }
  },
  ...
  _showTooltip () {
    if ( this.tooltip ) {
      this._showTooltipID = setTimeout(() => {
        ...
        let labelOffset = this.$label.getBoundingClientRect().left;
        let textOffset = this.$text.getBoundingClientRect().left;
        let offset = textOffset - labelOffset - 10;
