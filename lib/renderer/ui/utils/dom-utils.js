...
DomUtils.installDownUpEvent = function (element) {
  function _trackDocument(movefn, upfn) {
    _mouseState.movefn = movefn;
    _mouseState.upfn = upfn;
    document.addEventListener('mousemove', movefn);
    document.addEventListener('mouseup', upfn);
  }

  function _untrackDocument() {
    document.removeEventListener('mousemove', _mouseState.movefn);
    document.removeEventListener('mouseup', _mouseState.upfn);
    _mouseState.movefn = null;
    _mouseState.upfn = null;
  }
...
//
DomUtils.getParentTabIndex = function ( element ) {
  let parent = element.parentElement;
  while ( parent ) {
    if (
...
//
DomUtils.focusParent = function ( element ) {
...
//
DomUtils.getFirstFocusableChild = function ( element ) {
  if (
  ...
  for ( let i = 0; i < element.children.length; ++i ) {
    let childEL = DomUtils.getFirstFocusableChild(element.children[i]);
