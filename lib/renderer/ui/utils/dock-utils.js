...
let DockUtils = {
  ...
  dragstart ( dataTransfer, tabEL ) {
    ...
    if ( Ipc.sendToWindows ) {
      Ipc.sendToWindows('panel:dragstart', _draggingInfo, Ipc.selfExcluded);
      Ipc.sendToWindows('editor:dragstart');
    }
  },
  ...
  reset () {
    ...
    if ( DockUtils.root._dockable ) {
      ...
      DomUtils.fire(DockUtils.root, 'resize');
    }
  },
  ...
  loadLayout ( anchorEL, cb ) {
    Ipc.sendRequestToCore( 'window:query-layout', (layout, needReset) => {
      if ( !layout ) {
        if (cb) cb ( false );
        return;
      }
      ...
      DockUtils.resetLayout( anchorEL, layout, () => {
        if (cb) cb ( needReset );
      });
    });
  },

  resetLayout ( anchorEL, layoutInfo, cb ) {
    _layouting = true;
  ...
  saveLayout () {
    ...
    window.requestAnimationFrame ( () => {
      Ipc.sendToCore('window:save-layout', Panel.dumpLayout());
    });
  },
};
...
document.addEventListener('dragend', () => {
  ...
  _reset();
  if ( Ipc.sendToWindows ) {
    Ipc.sendToWindows( 'panel:dragend', Ipc.selfExcluded );
    Ipc.sendToWindows( 'editor:dragend' );
  }
});
...
ipcRenderer.on('editor:reset-layout', (event, layoutInfo) => {
  ...
  DockUtils.resetLayout( anchorEL, layoutInfo, () => {
    DockUtils.reset();
  });
});
