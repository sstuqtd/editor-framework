...
DockUtils.loadLayout = function (mainDock, cb) {
  Ipc.sendToMain('editor:window-query-layout', (err, layout) => {
    if ( err ) {
      if (cb) {
        cb (err);
      }
...
DockUtils.resetLayout = function (mainDock, info, cb) {
  ...
  Async.waterfall([
    ...
    // load all panels
    (frameELs, next) => {
      let doRun = EditorR.argv && EditorR.argv.panelID && EditorR.argv.panelArgv;
      ...
      frameELs.forEach(frameEL => {
        ...
        if ( frameEL._info.ui === 'polymer' ) {
          ...
          // run if we need
          if ( doRun && EditorR.argv.panelID === frameEL.id ) {
            if ( frameEL.run ) {
              frameEL.run(EditorR.argv.panelArgv);
            }
          }
    ...
    // need reset
    (next) => {
      // FIXME: Currently if we restore from snapshot, and we try to re-calculate the window, the panel size will become different
      // TODO: Actually no matter how, we should always re-calculate the layout
      // if this is not a snapshot, it means we need to re-calculate the size for each panel to fit the window
      if ( !info.snapshot ) {
        DockUtils.reset();
      }
      next();
