...
(() => {
  ...
  /**
   * Send message to core with by package name and channel short name
   * @method sendToPackage
   * @param {string} pkgName - the package name
   * @param {string} channel - the channel to send
   * @param {...*} [arg] - whatever arguments the channel needs
   */
  Editor.sendToPackage = function (pkgName, channel, ...args) {
    if ( typeof channel !== 'string' ) {
      Editor.error('The channel must be provided');
      return;
    }

    ipcRenderer.send.apply( ipcRenderer, [`${pkgName}:${channel}`, ...args] );
  };
  ...
  ipcRenderer.on('editor:sendreq2page', (event, channel, sessionId, args) => {
    let called = false;
