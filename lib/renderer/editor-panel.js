    ...
    Editor.sendRequestToCore('panel:query-info', panelID, ( panelInfo ) => {
      ...
      EditorUI.import( framePath, ( err ) => {
        ...
        if ( panelInfo.shortcuts ) {
          ...
          for ( let shortcut in panelInfo.shortcuts ) {
            if ( shortcut.length > 1 && shortcut[0] === '#' ) {
              let elementID = shortcut.substring(1);
              let subElement = frameEL.$[elementID];

              if ( !subElement ) {
                Editor.warn(`Failed to register shortcut for element #${elementID}, can not find it.`);
                continue;
              }

              let subShortcuts = panelInfo.shortcuts[shortcut];
