...
let EditorWindow = {
  ...
  load ( url, argv ) {
    Editor.sendToCore( 'window:load', url, argv, Editor.requireIpcEvent );
  },

  resize ( w, h ) {
    Editor.sendToCore( 'window:resize', w, h, Editor.requireIpcEvent );
  },

  center () {
    Editor.sendToCore( 'window:center', Editor.requireIpcEvent );
  },
};
