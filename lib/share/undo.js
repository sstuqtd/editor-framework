...
class Command {
  ...
  undo () { Console.warn('Please implement undo function in your command'); }
  redo () { Console.warn('Please implement redo function in your command'); }
  dirty () { return true; }
}
...
class UndoList extends EventEmitter {
  ...
  add ( id, info ) {
    ...
    if ( !ctor ) {
      Console.error( `Can not find undo command ${id}, please register it first` );
      return;
    }
  ...
  collapseTo ( index ) {
    ...
    if ( index > this._position || index < 0 ) {
      Console.warn(`Can not collapse undos to ${index}`);
      return;
    }
...
let Undo = {
  undo () {
    if ( Platform.isRendererProcess ) {
      Ipc.sendToCore('undo:perform-undo');
      return;
    }
  ...
  redo () {
    if ( Platform.isRendererProcess ) {
      Ipc.sendToCore('undo:perform-redo');
      return;
    }
  ...
  add ( id, info ) {
    if ( Platform.isRendererProcess ) {
      Ipc.sendToCore('undo:add', id, info );
      return;
    }
  ...
  collapseTo ( index ) {
    if ( Platform.isRendererProcess ) {
      Ipc.sendToCore('undo:collapse', index );
      return;
    }
  ...
  dirty () {
    if ( Platform.isRendererProcess ) {
      return Ipc.sendToCoreSync('undo:dirty');
    }
  ...
  setCurrentDescription ( desc ) {
    if ( Platform.isRendererProcess ) {
      return Ipc.sendToCoreSync('undo:set-desc', desc);
    }
