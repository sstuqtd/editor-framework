...
Ipc.on( '_selection:selected', ( type, ids ) => {
  let helper = _helpers[type];
  if ( !helper ) {
    Editor.error('Can not find the type %s for selection, please register it first', type);
    return;
...
Ipc.on( '_selection:unselected', ( type, ids ) => {
  let helper = _helpers[type];
  if ( !helper ) {
    Editor.error('Can not find the type %s for selection, please register it first', type);
    return;
...
Ipc.on( '_selection:activated', ( type, id ) => {
  let helper = _helpers[type];
  if ( !helper ) {
    Editor.error('Can not find the type %s for selection, please register it first', type);
    return;
...
Ipc.on( '_selection:deactivated', ( type, id ) => {
  unused(id);
...
Ipc.on( '_selection:hoverin', ( type, id ) => {
  let helper = _helpers[type];
  if ( !helper ) {
    Editor.error('Can not find the type %s for selection, please register it first', type);
    return;
...
Ipc.on( '_selection:hoverout', ( type, id ) => {
  unused(id);
...
Ipc.on( '_selection:context', ( type, id ) => {
  let helper = _helpers[type];
  if ( !helper ) {
    Editor.error('Can not find the type %s for selection, please register it first', type);
    return;
...
Ipc.on( '_selection:patch', ( type, srcID, destID ) => {
  let helper = _helpers[type];
  if ( !helper ) {
    Editor.error('Can not find the type %s for selection, please register it first', type);
    return;
