...
function _sendToAll (channel, type, ...args) {
  if ( type === 'local' ) {
    return;
  }

  // send _selection:xxx for sync selection data exclude self
  Editor.sendToAll.apply( Editor, [`_${channel}`, type, ...args, Editor.selfExcluded] );

  // send selection:xxx for user
  Editor.sendToAll.apply( Editor, [channel, type, ...args] );
}
