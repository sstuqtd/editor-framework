...
let Selection = {
  ...
  confirmed ( type ) {
    ...
    if ( !helper ) {
      Console.error('Can not find the type %s for selection, please register it first', type);
      return false;
    }
  ...
  select ( type, id, unselectOthers, confirm ) {
    ...
    if ( id && typeof id !== 'string' && !Array.isArray(id) ) {
      Console.error('The 2nd argument for Editor.Selection.select must be string or array');
      return;
    }
  ...
  setContext ( type, id ) {
    ...
    if ( id && typeof id !== 'string' ) {
      Console.error('The 2nd argument for Editor.Selection.setContext must be string');
      return;
    }
...
function _sendToAll (message, type, ...args) {
  ...
  Ipc.sendToAll.apply( null, [`_${message}`, type, ...args, Ipc.selfExcluded] );
...
if ( Platform.isRendererProcess ) {
  (() => {
    let results = Ipc.sendToCoreSync('selection:get-registers');
