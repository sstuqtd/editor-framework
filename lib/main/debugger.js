 ...
 * @method toggle
 */
Debugger.toggleVorpal = function () {
  if ( _vorpalRunning ) {
    Debugger.stopVorpal();
  } else {
    Debugger.startVorpal();
  }

  return _vorpalRunning;
};

Debugger.startVorpal = function () {
  _vorpalRunning = true;
  _vorpal.show();
  _vorpal.exec('debug');
};

Debugger.stopVorpal = function () {
  _vorpalRunning = false;
  _vorpal.hide();
};
