 ...
 */
Object.defineProperty(EditorM, 'watcher', {
  enumerable: true,
  get () { return _packageWatcher; }
});
...
EditorM.watchPackages = function ( cb ) {
  ...
  _packageWatcher
    .on('add', path => {
      ...
      // _packageWatcher.add(path); // DISABLE

      // NOTE: we should wait for 100ms to make sure the folder is copied
      setTimeout(() => {
        Package.load(path);
      }, 100);
    })
    ...
    .on('unlink', path => {
      ...
      // NOTE: do not use _packageWatcher.unwatch(path), it means do not watch the path
      Package.unload(path);
    })
    .on('unlinkDir', path => {
...
EditorM.reset = function () {
  ...
  // reset i18n method
  i18n.clear();
  i18n.extend(require(`../../static/i18n/${EditorM.lang}`));
