...
const Protocol = require('./editor-protocol');
      ...
      next => {
        ...
        if ( info.reloadPage ) {
          for ( let panelName in packageInfo.panels ) {
            ...
            Ipc.sendToWindows( 'panel:out-of-date', panelID );
          }
...
Editor.init = function ( opts ) {
  ...
  if ( profileInfo ) {
    for ( let name in profileInfo ) {
      ...
      Editor.Profile.register( name, profileInfo[name] );
    }
  }
...
Editor.reset = function () {
  ...
  // reset i18n method
  i18n.clear();
  i18n.extend(require(`../../static/i18n/${Editor.lang}.js`));
