...
Editor.init = function ( opts ) {
  ...
  // NOTE: 18n register must before main-menu, so that main-menu can translate by i18n module
...
ipcMain.on('console:log', function () {
  let args = [].slice.call( arguments, 1 );
  Editor.log.apply(Editor,args);
});

ipcMain.on('console:success', function () {
  let args = [].slice.call( arguments, 1 );
  Editor.success.apply(Editor,args);
});

ipcMain.on('console:failed', function () {
  let args = [].slice.call( arguments, 1 );
  Editor.failed.apply(Editor,args);
});

ipcMain.on('console:info', function () {
  let args = [].slice.call( arguments, 1 );
  Editor.info.apply(Editor,args);
});

ipcMain.on('console:warn', function () {
  let args = [].slice.call( arguments, 1 );
  Editor.warn.apply(Editor,args);
});

ipcMain.on('console:error', function () {
  let args = [].slice.call( arguments, 1 );
  Editor.error.apply(Editor,args);
});

ipcMain.on('console:query', function ( event, reply ) {
  reply(_logs);
});

ipcMain.on('_console:clear', function () {
  Editor.clearLog();
});
