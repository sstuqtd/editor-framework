...
Package.load = function ( path, cb ) {
  ...
  Async.series([
    ...
    next => {
      // redir
      pjsonObj._path = path;
      pjsonObj._destPath = path;

      if ( pjsonObj['entry-dir'] ) {
        pjsonObj._destPath = Path.join(pjsonObj._path, pjsonObj['entry-dir']);
      }
      ...
      for ( let name in panels ) {
        ...
        if ( !Fs.existsSync(Path.join(pjsonObj._destPath,panelInfo.main)) ) {
