...
Package.load = function ( path, opts, cb ) {
  ...
  Async.series([
    ...
    next => {
      ...
      Ipc.sendToWins('package:loaded', packageObj.name);
      next ();
    },
...
Package.unload = function ( path, cb ) {
  ...
  Ipc.sendToWins('package:unloaded', packageObj.name);
...
ipcMain.on('package:query-infos', (event) => {
...
ipcMain.on('package:query-info', (event, name) => {
