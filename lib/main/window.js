...
class Window extends EventEmitter {
  constructor ( name, options ) {
    ...
    this.nativeWin.webContents.on('did-finish-load', () => {
      ...
      // Ipc.sendToCore('window:reloaded', this);
    });
   ...
   */
  sendRequestToPage (message, ...args) {
    ...
    this.sendToPage('editor:sendreq2page', message, sessionId, args);
    return sessionId;
  }
 ...
 */
Object.defineProperty(Window.prototype, 'isMainWindow', {
  enumerable: true,
  get() {
   ...
   */
Object.defineProperty(Window.prototype, 'isFocused', {
  enumerable: true,
  get () {
 ...
 */
Object.defineProperty(Window.prototype, 'isLoaded', {
  enumerable: true,
  get () {
    return this._loaded;
  }
...
ipcMain.on('editor:sendreq2page:reply', ( event, sessionId, args ) => {
  ...
  if ( !editorWin ) {
    Console.warn('editor:sendreq2page:reply failed: Can not find the window.' );
    return;
  }
...
ipcMain.on('window:resize', ( event, w, h, useContentSize ) => {
  let win = BrowserWindow.fromWebContents( event.sender );
  let editorWin = Window.find(win);
  if ( !editorWin ) {
    Console.warn('Failed to focus, can not find the window.');
