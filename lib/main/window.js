...
class Window extends EventEmitter {
  constructor ( name, options ) {
    ...
    if ( _windowsProfile.windows && _windowsProfile.windows[name] ) {
      this._state = _windowsProfile.windows[name];
    }
    ...
    this.nativeWin.on('close', event => {
      ...
      Window._commitWindowStates();
      Window._saveWindowStates();
    });
  ...
  commitWindowState () {
    ...
    this._state.x = winBounds.x;
    this._state.y = winBounds.y;
    this._state.width = winBounds.width;
    this._state.height = winBounds.height;
  }
  ...
  restorePositionAndSize () {
    ...
    if ( _windowsProfile.windows && _windowsProfile.windows[this.name] ) {
      let winInfo = _windowsProfile.windows[this.name];
      winPosX = winInfo.x;
      winPosY = winInfo.y;
  ...
  _addPanel ( panelID ) {
    if ( this._panels.indexOf(panelID) !== -1 ) {
      // Editor.error(`The ${panelID} already exists on the window.`);
      return;
    }
  ...
  _removePanel ( panelID ) {
    let idx = this._panels.indexOf(panelID);
    if ( idx === -1 ) {
      // Editor.error(`The ${panelID} didn't exist on the window.`);
      return;
    }
  ...
  static _saveWindowStates () {
    ...
    //
    _windowsProfile.version = profileVersion;
    _windowsProfile.windows = {};

    for ( let i = 0; i < _windows.length; ++i ) {
      let editorWin = _windows[i];

      _windowsProfile.windows[editorWin.name] = editorWin._state;
    }
    _windowsProfile.save();
  }

  // NOTE: this will be invoked in Editor.App.run after Profile finish registerred
  static _loadWindowStates () {
    _windowsProfile = Profile.load( 'layout.windows', 'local', {
      version: profileVersion,
      windows: {},
    });

    // reset layout if the version is not the same
    if ( _windowsProfile.version !== profileVersion ) {
      _windowsProfile.windows = {};
    }
  }

  // Commit all opened window's state
  static _commitWindowStates () {
    for ( let i = 0; i < _windows.length; ++i ) {
      let editorWin = _windows[i];
...
ipcMain.on('editor:window-query-layout', ( event ) => {
  ...
  let layout = null;
  let winState = editorWin._state;
  if ( winState && winState.layout ) {
    layout = winState.layout;
  }
...
ipcMain.on('editor:window-save-layout', ( event, layoutInfo ) => {
  ...
  editorWin._state.layout = layoutInfo;
...
ipcMain.on('editor:window-remove-all-panels', ( event ) => {
  let win = BrowserWindow.fromWebContents( event.sender );
  let editorWin = Window.find(win);
  if ( !editorWin ) {
    event.reply();
