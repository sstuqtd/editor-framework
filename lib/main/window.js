...
class Window extends EventEmitter {
  constructor ( name, options ) {
    ...
    this.nativeWin.webContents.on('did-finish-load', () => {
      ...
      // Ipc.sendToMain('window:reloaded', this);
    });
  ...
  send ( message, ...args ) {
    ...
    if ( !opts ) {
      args = [message, ...args];
      if ( this._send.apply ( null, args ) === false ) {
        Console.failed( `sendToMain "${message}" failed, not responded.` );
      }
      return;
    }

    Ipc._registerReplyIpc( message );
    let sessionId = Ipc._addSession(`${this.nativeWin.id}@main`, opts.reply, opts.timeout, this);
...
ipcMain.on('window:query-layout', ( event ) => {
  let win = BrowserWindow.fromWebContents( event.sender );
  let editorWin = Window.find(win);
  if ( !editorWin ) {
