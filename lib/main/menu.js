...
function _expandMenuTemplate ( tmpl, index ) {
  ...
  for ( let i = 0; i < pathNames.length-1; i++ ) {
    ...
    if ( !parentTmpl.submenu || parentTmpl.type !== 'submenu' ) {
      Console.warn( `Can not add menu in ${itemTmpl.path}, the ${curPath} already used` );
      return;
    }
...
function _getMenuItem ( nativeMenu, path, createIfNotExists ) {
  ...
  for (let i = 0; i < pathNames.length; i++) {
    ...
    if (menuItem) {
      ...
      if ( !menuItem.submenu || menuItem.type !== 'submenu' ) {
        Console.warn( `Can not add menu in ${path}, the ${curPath} already used` );
        return null;
      }
...
function _convert ( submenuTmpl, index, webContents ) {
  ...
  if ( template.message ) {
    ...
    if ( template.panel ) {
      ...
      template.click = () => {
        setImmediate(() => {
          Ipc.sendToCore.apply(null, args);
        });
      };
    }
  ...
  // parse command
    ...
    if ( !fn || typeof fn !== 'function' ) {
      Console.error(
        `Failed to add menu item '${itemName}', can not find global function ${template.command} in main process for 'command'.`
      );
      // return true to remote the menu item
      return true;
    }
  ...
  if ( template.path ) {
    ...
    if ( template.label ) {
      Console.warn(`Skip label "${template.label}" in menu item "${template.path}"`);
    }
...
class Menu {
  ...
  remove ( path ) {
    ...
    if ( !removed ) {
      Console.error(`Failed to remove menu in path: ${path}, can not find it` );
      return false;
    }
  ...
  set ( path, options ) {
    ...
    if ( menuItem.type === 'separator' ) {
      Console.error(`Menu item ${path} is a separator`);
      return false;
    }
  ...
  static register ( name, fn, force ) {
    if ( typeof fn !== 'function' ) {
      Console.warn(`Can not register menu ${name}, fn must be a function`);
      return;
    }

    if ( !force && _menus[name] ) {
      Console.warn(`Can not register menu ${name}, already exists`);
      return;
    }
  ...
  static unregister ( name ) {
    if ( !_menus[name] ) {
      Console.warn(`Can not find menu ${name}`);
      return;
    }
