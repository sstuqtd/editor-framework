...
Ipc.sendToMainWin = function (message, ...args) {
  ...
  // TODO: add _send2win which will process callback args

  mainWin._send.apply( mainWin, args );
};
...
Ipc.sendToPanel = function (panelID, message, ...args) {
  ...
  if ( panelInfo.type === 'simple' ) {
    win._send.apply( win, [message, ...args] );
    return;
  }

  //
  win._send.apply( win, ['editor:ipc-main2panel', panelID, message, ...args] );
};
...
Ipc.cancelRequest = function (sessionId) {
  if ( sessionId.indexOf('@main') !== -1 ) {
    Console.warn(`Can not cancel session ${sessionId}, use "Editor.Window.cancelRequest" instead.`);
    return;
  }
...
Ipc._registerReplyIpc = _registerReplyIpc;
Ipc._addSession = _addSession;
Ipc._closeSession = function (sessionId) {
  delete _id2callback[sessionId];
};
...
function _registerReplyIpc ( message, win ) {
  let replyMessage = `${message}:reply`;

  // already registered
  if ( ipcMain.listeners(replyMessage).length > 0 ) {
    return;
  }

  ipcMain.on(replyMessage, (event, ...args) => {
    let opts = _popOptions(args);
    ...
    if (cb) {
      ...
      // TODO: we can not store win here, cause ipcMain may already registered
      if (win) {
...
function _main2mainOpts (message, ...args) {
  ...
  if ( opts && opts.waitForReply ) {
    event.reply = function (...replyArgs) {
      ...
      replyArgs = [`${message}:reply`, event, ...replyArgs, replyOpts];
...
function _renderer2mainOpts (event, message, ...args) {
  ...
  if ( opts && opts.waitForReply ) {
    event.reply = function (...replyArgs) {
      ...
      replyArgs = [`${message}:reply`, ...replyArgs, replyOpts];
