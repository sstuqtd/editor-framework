...
Ipc.sendToAll = function (message, ...args) {
  if (args.length) {
    ...
    let opt = _popOptions(args);

    // check options
    if (opt && opt.excludeSelf) {
      excludeSelf = true;
    }
...
Ipc.sendToMainWin = function (message, ...args) {
  ...
  mainWin.send.apply( mainWin, [message, ...args] );
};
...
Ipc.sendToPanel = function (panelID, message, ...args) {
  ...
  //
  win.send.apply( win, ['editor:send2panel', panelID, message, ...args] );
};
...
ipcMain.on('editor:send2mainwin', (event, message, ...args) => {
  ...
  if (args.length) {
    // discard event arg
    mainWin.send.apply( mainWin, [event, message, ...args] );
  } else {
    mainWin.send( message );
  }
});
