...
Ipc.sendToPanel = function (panelID, message, ...args) {
  ...
  //
  win._sendByChannel.apply( win, ['editor:ipc-main2panel', message, panelID, ...args] );
};
...
function _renderer2panelOpts (event, message, panelID, ...args) {
  if ( args.length === 0 ) {
...
ipcMain.on('editor:ipc-renderer2panel', (event, message, panelID, ...args) => {
