...
function _main2main ( channel ) {
  let event = {
    senderType: 'main',
    sender: {
      send () {
        _main2main.apply ( null, arguments );
      }
    }
  };
...
function _renderer2main ( event, channel ) {
...
function _renderer2renderers ( event, channel ) {
...
Editor.sendToCore = function () {
  if ( _main2main.apply ( null, arguments ) === false ) {
...
Editor.sendToAll = function () {
  if (arguments.length > 1) {
    ...
    // send
    if (toSelf) {
      _main2main.apply(null, args); // sendToCore (dont require receiver)
    }
    Editor.sendToWindows.apply(Editor, args);
  ...
  _main2main(arguments[0]); // sendToCore (dont require receiver)
...
//   if ( !_renderer2main.apply ( ipcMain, arguments ) ) {
...
ipcMain.on('editor:send2all', function () {
  _renderer2main.apply(ipcMain, arguments);
  _renderer2renderers.apply(ipcMain, arguments);
});
...
ipcMain.on('editor:sendreq2core', function (event, channel, sessionId, args) {
  let called = false;
  ...
  args.unshift(event, channel, _replyCallback);

  if ( !_renderer2main.apply(ipcMain, args) ) {
