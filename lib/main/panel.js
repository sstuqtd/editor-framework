...
Panel.open = function ( panelID, argv ) {
  ...
  if ( editorWin ) {
    ...
    Ipc.sendToPanel( panelID, 'panel:run', argv );
    editorWin.show();
    editorWin.focus();
    return;
  }
...
function _undock ( panelID ) {
  ...
  if ( editorWin ) {
    editorWin.send( 'panel:undock', panelID );
...
ipcMain.on('panel:query-info', ( event, panelID ) => {
  if ( !panelID ) {
...
ipcMain.on('panel:wait-for-close', ( event, panelID ) => {
  Panel.close( panelID );
  event.reply();
});
