...
/**
 * Open a panel and pass argv to it. The argv will be sent as argument in `editor:panel-run` message in page-level
 * @method open
...
Panel.open = function ( panelID, argv ) {
  ...
  if ( editorWin ) {
    // if we found the window, send editor:panel-run to it
    Ipc.sendToPanel( panelID, 'editor:panel-run', argv );
    editorWin.show();
    editorWin.focus();
    return;
  }
...
ipcMain.on('editor:panel-query-info', ( event, panelID ) => {
  ...
  //
  event.reply(panelInfo);
});

ipcMain.on('editor:panel-ready', ( event, panelID ) => {
