...
App.on('ready', () => {
  ...
  Commander
    ...
    .option('--test-renderer', 'Run tests in renderer' )
    .option('--debug-test', 'Run test in debug mode (It will not quit the test, and open the devtools to help you debug it)' )
    .option('--reporter <name>', 'Mocha reporter, default is \'spec\'' )
    .option('--packages <package-name>', 'Load packages, separated by \',\'', parseList)
    ;
  ...
  let runTest = Commander.test || Commander.testRenderer || Commander.debugTest;
  if ( !runTest ) {
    console.log( Chalk.magenta('===== Initializing Editor =====') );
  }
  if ( runTest && !Commander.debugTest ) {
    // silent the console output when testing
    Winston.remove(Winston.transports.Console);
  }
  require('./editor-init');
