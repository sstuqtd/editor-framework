...
//
Mocha.reporters['child-process'] = require('./main/child-process-reporter');
...
let Tester = {
  ...
  runRenderer ( path, opts, cb ) {
    ...
    //
    ipcMain.on('mocha-reload', () => {
      // reset reporter
      inst = _resetRendererReporter(inst);
    });

    //
    ipcMain.on('mocha-done', (event, failures) => {
      if ( opts.detail ) {
        // reset reporter
        inst = _resetRendererReporter(inst);
  ...
  run ( path, opts ) {
    ...
    Editor.Menu.register('main-menu', require('./main/main-menu'), true);
