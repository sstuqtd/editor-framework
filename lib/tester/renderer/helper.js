...
_.assign(Helper, {
  ...
  focus ( target ) {
    Editor.UI.FocusMgr._setFocusElement(target);
  ...
  blur () {
    Editor.UI.FocusMgr._setFocusElement(null);
  ...
  loadTemplate ( url, id, cb ) {
    this.importHTML(url, ( el ) => {
      if ( id ) {
        let tmpl = el.getElementById(id);
        if ( !tmpl ) {
          throw new Error(`Can not find template by id ${id}`);
        }
